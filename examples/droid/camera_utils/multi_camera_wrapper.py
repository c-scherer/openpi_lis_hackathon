...
class MultiCameraWrapper:
    def __init__(self, camera_kwargs={}):
        ...
        self.camera_dict = {cam.serial_number: cam for cam in realsense_cameras}
        self.camera_dict.update({cam.serial_number: cam for cam in zed_cameras})
    ...
    def start_recording(self, recording_folderpath):
        subdir = os.path.join(recording_folderpath, "BAG")
        if not os.path.isdir(subdir):
            os.makedirs(subdir)
        for cam in self.camera_dict.values():
            ext = getattr(cam, "preferred_recording_extension", ".bag")
            filepath = os.path.join(subdir, f"{cam.serial_number}{ext}")
